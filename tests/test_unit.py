...
def mock_authenticated_user(monkeypatch, user_data=None):
    ...
    monkeypatch.setattr(
        ...
        "services.auth_service.get_token",
...
class TestAuth:
    ...
    def test_logout_success(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            "services.auth_service.AuthService.logout",


            mock_auth_service_logout)
...
class TestUser:
    ...
    def test_user_list(self, monkeypatch):
        ...
        mock_authenticated_user(
            monkeypatch, {"user_id": 1, "departement": "Gestion"})
        monkeypatch.setattr(

            "services.user_services.UserService.get_users",
...
class TestClient:
    ...
    def test_client_create_success(self, monkeypatch):
        ...
        # Mock authentification
        mock_authenticated_user(
            monkeypatch, {"user_id": 1, "departement": "Commercial"})
        monkeypatch.setattr(
...
class TestContract:
    ...
    def test_contract_create_success(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            "services.contract_services.ContractService.create_contract",
    ...
    def test_contract_update_success(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            "services.contract_services.ContractService.update_contract",

            mock_update_contract)

        # Test de mise à jour avec signature
        result = runner.invoke(
            contract, ['update', '--contract-id', '1', '--sign'])
    ...
    def test_contract_list_success(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            "services.contract_services.ContractService.get_contract_list",

            mock_get_contract_list)
    ...
    def test_contracts_not_sign_success(self, monkeypatch):
        ...
        mock_authenticated_user(
            monkeypatch, {"user_id": 1, "departement": "Commercial"})
        monkeypatch.setattr(
            "services.contract_services.ContractService."
            "get_contract_list_not_sign",
            mock_get_contract_list_not_sign)
    ...
    def test_contracts_not_sign_empty(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "services.contract_services.ContractService."
            "get_contract_list_not_sign", mock_get_contract_list_not_sign)
    ...
    def test_contracts_not_paid_success(self, monkeypatch):
        ...
        def mock_get_contract_list_not_fully_paid(self):
            return (True, fake_contracts,
                    "Contrats non entièrement payés récupérés")

        # Mock authentification
        mock_authenticated_user(
            monkeypatch, {"user_id": 1, "departement": "Commercial"})
        monkeypatch.setattr("services.contract_services.ContractService."
                            "get_contract_list_not_fully_paid",
                            mock_get_contract_list_not_fully_paid)
    ...
    def test_contract_commands_import(self):
        """Test pour forcer l'import du module contract_commands
         pour la couverture"""
        from cli.commands.contract_commands import contract
        ...
        expected_commands = ['create', 'update', 'list',
                             'contracts-not-sign', 'contracts-not-paid']
    ...
    def test_contract_access_denied_wrong_department(self, monkeypatch):
        ...
        mock_authenticated_user(
            monkeypatch, {"user_id": 1, "departement": "Support"})

        result = runner.invoke(contract, ['create'])
        assert ("Accès refusé" in result.output or
                "access" in result.output.lower())
...
class TestEvent:
    ...
    def test_event_create_success(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            "services.event_services.EventService.create_event",
...
class TestAuthDecorators:
    ...
    def test_require_auth_without_login(self, monkeypatch):
        ...
        result = runner.invoke(client, ['get-clients'])
        assert "Vous devez être connecté" in result.output or "Vous \
              n'êtes pas connecté" in result.output
    ...
    def test_require_departement_wrong_dept(self, monkeypatch):
        ...
        # Un utilisateur Support ne peut pas créer d'utilisateur
        # (réservé à Gestion)
...
class TestEventCommands:
    ...
    def test_event_create_failure(self, monkeypatch,
                                  mock_auth_success):
    ...
    def test_event_create_invalid_support_id(self,
                                             monkeypatch,
                                             mock_auth_success):
        ...
        def mock_create_event(self,
                              ...
                              support_id):
            return False, (
                "ID du support n'est pas unmembre de l'équipe support")
    ...
    def test_event_update_success_gestion(self,
                                          monkeypatch,
                                          mock_auth_success):
        ...
        monkeypatch.setattr(
            ...
            "services.event_services.EventService.update_event",
    ...
    def test_event_update_success_support(self, monkeypatch,
                                          mock_auth_support):
    ...
    def test_event_update_not_found(self, monkeypatch,
                                    mock_auth_success):
    ...
    def test_event_update_invalid_date_format(self, monkeypatch,
                                              mock_auth_success):
    ...
    def test_event_update_invalid_attendees(self, monkeypatch,
                                            mock_auth_success):
    ...
    def test_event_update_negative_attendees(self, monkeypatch,
                                             mock_auth_success):
    ...
    def test_event_update_invalid_support_id(self, monkeypatch,
                                             mock_auth_success):
    ...
    def test_event_update_end_date_before_start(self, monkeypatch,
                                                mock_auth_success):
        """Test de mise à jour avec date de fin
        antérieure à date de début"""
        ...
        runner = CliRunner()
        # Date de début puis date de fin antérieure, puis date de fin
        # correcte pour corriger
        result = runner.invoke(event, [
            'update',
            '--event-id', '1'
        ], input='2024-12-25\n2024-12-24\n2024-12-26\n\n\n\n\n')
        ...
        assert result.exit_code == 0
        assert "La date de fin doit être postérieure" in result.output
        assert "à la date de début" in result.output

    def test_assigned_events_with_events(self, monkeypatch,
                                         mock_auth_success):
        """Test de récupération d'événements assignés
        avec des événements"""
        class MockEvent:
        ...
        mock_events = [
            MockEvent(1, "2024-12-25 10:00:00",
                      "2024-12-25 18:00:00", 50, "Salle A"),
            MockEvent(2, "2024-12-26 14:00:00",
                      "2024-12-26 17:00:00", 30, "Salle B")
        ]

        def mock_get_events(self):
            return True, mock_events, "2 événement(s) attribué(s)"

        monkeypatch.setattr("services.event_services.EventService."
                            "get_events_by_support_contact_id",
                            mock_get_events)
    ...
    def test_event_create_unauthorized_commercial(self, monkeypatch):
        ...
        par département Commercial (non autorisé)"""
        def mock_get_current_user():
            return {"user_id": 3, "username": "commercial",
    ...
    def test_event_update_unauthorized_commercial(self, monkeypatch):
        """Test de mise à jour d'événement par
        département Commercial (non autorisé)"""
        def mock_get_current_user():
            return {"user_id": 3,
                    "username": "commercial",
                    "departement": "Commercial"}
    ...
    def test_event_commands_import(self):
        ...
        from cli.commands.event_commands import (
            event, create, event_update,
            get_assign_events_by_support_contact_id)
...
class TestInitDB:
    ...
    def test_init_db_success_first_time(self, monkeypatch, capfd):
        """Test d'initialisation DB avec insertion
        des départements (première fois)"""
        from database.init_db import init_db
        ...
        class MockConnection:
            def execute(self, query, values=None):
                if values is None:  # SELECT query
                    # Retourner un résultat vide pour simuler
        ...
        assert ("Gestion" in captured.out or "Commercial" in captured.out
                or "Support" in captured.out)
        ...
        class MockConnection:
            def execute(self, query, values=None):
                if values is None:  # SELECT query
                    ...
                    return MockResult([("Gestion",), ("Commercial",),
                                       ("Support",)])
                else:  # INSERT query - ne devrait pas être appelé
                    raise AssertionError(
                        "INSERT ne devrait pas être appelé quand \
                            tous les départements existent")
    ...
    def test_init_db_partial_departments_exist(self, monkeypatch, capfd):
        """Test d'initialisation DB quand certains départements
        existent déjà"""
        from database.init_db import init_db
        ...
        captured = capfd.readouterr()
        assert "Départements ajoutés" in captured.out
        # Devrait ajouter Commercial et Support
        # (mais pas Gestion qui existe déjà)
    ...
    def test_init_db_database_error(self, monkeypatch):
        ...
        assert "Erreur de connexion à la base de données" in str(
            exc_info.value)
    ...
    def test_init_db_module_execution(self, monkeypatch, capfd):
        ...
        # Mais comme nous l'importons, __name__ sera le nom du module,
        # pas "__main__"
...
""")
    ...
    def test_init_db_integration_with_real_objects(self, monkeypatch):
        """Test d'intégration avec de vrais objets
        (sans connexion DB réelle)"""
        from database.init_db import init_db
...
class TestUserCommands:
    ...
    def test_user_create_success(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            "cli.commands.user_commands.get_departement_choice",


            mock_get_departement_choice)
        monkeypatch.setattr(

            "cli.commands.user_commands.get_departement_id_by_name",
    ...
    def test_user_create_invalid_employee_number(self, monkeypatch):
        ...
        assert ("Le matricule doit être un nombre entier valide"
                in result.output)
    ...
    def test_user_create_department_not_found(self, monkeypatch):
        ...
        assert result.exit_code == 0
        assert "❌ Erreur : Le département 'Gestion'" in result.output
        assert "n'a pas été trouvé." in result.output
    ...
    def test_user_list_success(self, monkeypatch):
        ...
        def mock_get_users(self):
            return [
                ...
                MockUser(2, 67890, "jane_smith",
                         "jane@epic.com", "Jane", "Smith")
            ]
...
class TestContractUpdate:
    ...
    def test_contract_update_success_commercial_unassigned_client(
            self, monkeypatch):
        """
    ...
    def test_contract_update_fail_commercial_other_client(self, monkeypatch):
        """
        Test échec: commercial tente de mettre à jour un contrat
        ...
        def mock_update_contract(self,
                                 ...
                                 paid_amount=None):
            return False, (
                "Vous ne pouvez mettre à jour que les contrats de vos clients."
                "Ce contrat appartient à un client assigné à un autre "
                "commercial (ID: 2)")
        ...
        assert result.exit_code == 0
        assert ("Ce contrat appartient à un client assigné "
                "à un autre commercial" in result.output)
    ...
    def test_contract_update_fail_contract_not_found(self, monkeypatch):
        ...
        result = runner.invoke(
            contract, ['update', '--contract-id', '999', '--sign'])
    ...
    def test_contract_update_gestion_user_paid_amount(self, monkeypatch):
        ...
        # Mock contract service
        def mock_update_contract(self, contract_id, user_id, user_departement,
    ...
    def test_contract_update_fail_paid_amount_too_high(self, monkeypatch):
        ...
        def mock_update_contract(self, contract_id, user_id, user_departement,
                                 sign=None, paid_amount=None):
            return False, (
                "Le montant payé (5000€) ne doit pas être supérieur "
                "au montant du contrat (2000€)")
        ...
        assert result.exit_code == 0
        assert ("ne doit pas être supérieur au montant du contrat"
                in result.output)
...
class TestContractServiceUpdate:
    ...
    def test_contract_service_update_assigns_commercial_to_unassigned_client(
            self, monkeypatch):
        """
        Test: assignation automatique d'un commercial à un client
        non assigné lors de la signature
        """
        ...
        monkeypatch.setattr(
            ...
            "database.dao.contract_dao.ContractDAO.get_contract_by_id",


            mock_get_contract_by_id)
        monkeypatch.setattr(

            "database.dao.client_dao.ClientDAO.get_client_by_id",

            mock_get_client_by_id)
        monkeypatch.setattr(

            "database.dao.contract_dao.ContractDAO.update_contract",

            mock_update_contract)
        monkeypatch.setattr(

            "database.dao.client_dao.ClientDAO.update_client",

            mock_update_client)
        monkeypatch.setattr(

            "services.sentry_service.log_contract_signature",

            mock_log_contract_signature)
        monkeypatch.setattr(
    ...
    def test_contract_service_update_permission_denied_wrong_commercial(
            self,
            monkeypatch):
        """
        ...
        assert ("Ce contrat appartient à un client assigné "
                "à un autre commercial" in message)
