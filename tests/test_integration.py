...
class TestIntegration:
    ...
    def test_full_user_workflow(self, monkeypatch):
        ...
        input_data = 'testuser\n123\ntest@test.com\nTest\nUser\npassword\npassword\nGestion\n'
        result = self.runner.invoke(epic, ['user', 'create'], input=input_data)
    ...
    def test_full_client_workflow(self, monkeypatch):
        ...
        result = self.runner.invoke(epic, ['client', 'create'], input=input_data)
    ...
    def test_authentication_workflow(self, monkeypatch):
        ...
        monkeypatch.setattr("services.auth_service.AuthService.login", mock_auth_login)
        monkeypatch.setattr("services.auth_service.AuthService.logout", mock_auth_logout)
        
        # Test login
        result = self.runner.invoke(epic, ['auth', 'login'], input='testuser\npassword\n')
        assert result.exit_code == 0
    ...
    def test_permission_system(self, monkeypatch):
        ...
        monkeypatch.setattr("services.auth_service.get_current_user_info", mock_get_current_user_info_none)
        ...
        monkeypatch.setattr("services.auth_service.get_current_user_info", mock_get_current_user_info_wrong_dept)
...
class TestErrorHandling:
    ...
    def test_missing_required_option(self, monkeypatch):
        ...
        assert "event-id" in result.output.lower() or "id" in result.output.lower()
