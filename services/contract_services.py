...
class ContractService:
    ...
    def create_contract(self, title, client_id, amount):
        ...
        Le contrat est créé avec un statut non signé (status=False) et sans commercial assigné.
        
        Args:
            title (str): Titre descriptif du contrat
            client_id (int): Identifiant du client auquel le contrat est rattaché
            amount (float): Montant total du contrat en euros
            
        Returns:
            tuple: (success, message)
                - success (bool): True si la création du contrat a réussi, False sinon
                - message (str): Message décrivant le résultat de l'opération
                
        Raises:
            Les exceptions sont capturées et loggées via Sentry, retournant un tuple d'erreur
        """
    ...
    def update_contract(self, contract_id,
        ...
        Cette méthode gère la signature et le paiement des contrats avec les règles suivantes :
        - Un commercial peut signer un contrat s'il est assigné au client ou si le client n'a pas de commercial
        - Seuls les utilisateurs du département "Gestion" peuvent modifier les montants payés
        - Les montants payés ne peuvent pas excéder le montant total du contrat
        ...
        Args:
            ...
            sign (bool, optional): Le contrat doit-il être signé. Defaults to None.
            paid_amount (float, optional): montant déjà payé. Defaults to None.
        ...
        if user_departement.lower() == "commercial":
            ...
            if contract.commercial_id is not None and contract.commercial_id != user_id:
                return False, f"Vous ne pouvez mettre à jour que les contrats de vos clients. " \
                             f"Ce contrat appartient à un client assigné à un autre commercial (ID: {contract.commercial_id})"
        ...
        if sign:
            ...
            # Si le client n'a pas de commercial assigné et que l'utilisateur est commercial,
            # on peut assigner le commercial au client lors de la signature
            if contract.commercial_id is None and user_departement.lower() == "commercial":
                # Mettre à jour le commercial du client
                self.client_dao.update_client(contract.client_id, {"commercial_id": user_id})

        # Gestion du paiement
        if paid_amount is not None and paid_amount > 0:
            if paid_amount > contract.amount:
                return False, f"Le montant payé ({paid_amount}€) ne doit pas être supérieur " \
                             f"au montant du contrat ({contract.amount}€)"
    ...
    def get_contract_list(self):
        ...
        Cette méthode retourne tous les contrats présents dans la base de données,
        peu importe leur statut (signé ou non signé).
        
        Returns:
            tuple: (success, contracts, message)
                - success (bool): True si la récupération a réussi, False sinon
                - contracts (list): Liste des contrats ou liste vide en cas d'erreur
                - message (str): Message décrivant le résultat de l'opération
    ...
    def get_contract_list_not_fully_paid(self):
        ...
        Returns:
            tuple: (success, contracts, message)
                - success (bool): True si la récupération a réussi, False sinon
                - contracts (list): Liste des contrats partiellement payés ou liste vide
                - message (str): Message décrivant le résultat de l'opération
